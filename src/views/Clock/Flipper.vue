...
export default class extends Vue {
  ...
  /* data */
  private flipType = 'down'

  private isFlipping = false

  private frontTextFromData: string | number = 0

  private backTextFromData: string | number = 1
  ...
  /* methods */
  private textClass(number: string | number) {
    return `number${number}`
  }

  private flip(type: string, front: string | number, back: string | number) {
    // 如果处于翻转中，则不执行
    if (this.isFlipping) {
      return
    }
  ...
  // 下翻牌
  private flipDown(front: string | number, back: string | number) {
    this.flip('down', front, back)
  }

  // 上翻牌
  private flipUp(front: string | number, back: string | number) {
    this.flip('up', front, back)
  }

  // 设置前牌文字
  private setFront(text: string | number) {
    this.frontTextFromData = text
  }

  // 设置后牌文字
  private setBack(text: string | number) {
